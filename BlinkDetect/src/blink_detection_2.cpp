...
/*    "eyes.xml*/
const char *cascade_name[1]={"xml/eyes.xml"};
//const char *cascade_name[1]={"xml/haarcascade_eye.xml"};
cv::Mat roiImg;
...
int blink_detection( void )
{
	...
	* */
    ...
    const char* input_name;
     ...
     img1 = imread( file_one, 1 );
     img2 = imread( file_two, 1 );
	...
	fprintf( stdout, "Opening Camera...\n");	
	int cam_open = Camera.open();
	if (!cam_open) 
	{
		fprintf( stderr, "Error opening the camera\n");		
		return 1;
	}
    ...
    if( 1 )
    {
        ...
        while(1)
        {
			...
			if (image_cam.empty())
			{
				fprintf( stderr, "no image read from camera -- terminating\n");
				break;
			}
			...
			*/
			...
			{		
	            ...
	            if(detect_and_draw(frame,cascade))
				{
					;			
				}	
