...
cout << "face casecade: " << face_cascade.empty() <<", eye cascade: " << eye_cascade.empty() << endl;
	// Check if everything is ok
	/*
	if (face_cascade.empty() || eye_cascade.empty() || !Camera.open())
	{
		cout<< "Error in template load or camera is not available" << endl;
		return 1;
	}
	* */
	...
	while (cv::waitKey(15) != 'q')
	{
		...
		resize(image_cam, image, size); //resize image
		...
		detectEye(image, eye_tpl, eye_bb);
